  ...
  return async function userInterceptor(ctx, next) {
    ...
    if (authToken) {
      ...
      if (res.corpid && res.userid) {
        ...
        // 此处使用redis进行保存
        const redis_token = await app.redis.get('foo').get(res.corpid + res.userid) // 获取保存的token
        if (authToken === redis_token) {
          ctx.locals.corpid = res.corpid
          ctx.locals.userid = res.userid
          await next()
        } else {
          ctx.body = { code: 99, message: '您的账号已在其他地方登录' }
        }
      } else {
        ctx.body = { code: 99, message: '登录状态已过期' }
      }
    } else {
      ctx.body = { code: 0, message: '请登陆后再进行操作' }
    }
  }
}
